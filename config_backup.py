...
class OSRMConfig:
    ...
    chunk_size: int = 5  # много по-малки chunks за локални сървъри (Route API)
    ...
    fallback_to_public: bool = True  # дали да използва публичен сървър при грешка
...
class CVRPConfig:
    ...
    # OR-Tools настройки
    time_limit_seconds: int = 1200  # 20 минути за 150-250 клиента
    first_solution_strategy: str = "PARALLEL_CHEAPEST_INSERTION"  # по-бърза за големи проблеми
    local_search_metaheuristic: str = "GUIDED_LOCAL_SEARCH"  # локално търсене
    log_search: bool = True  # логиране на процеса
